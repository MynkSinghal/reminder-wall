...
// ── Font loading (Bricolage Grotesque via Google Fonts) ─────────────────────
let _fontSemi:  ArrayBuffer | null = null;
let _fontExtra: ArrayBuffer | null = null;

async function fetchGFont(weight: number): Promise<ArrayBuffer> {
  ...
  if (!url) throw new Error('woff2 not found for weight ' + weight);
  return fetch(url).then(r => r.arrayBuffer());
}

async function getFonts(): Promise<{ semi: ArrayBuffer; extra: ArrayBuffer }> {
  if (!_fontSemi)  _fontSemi  = await fetchGFont(600);
  if (!_fontExtra) _fontExtra = await fetchGFont(800);
  return { semi: _fontSemi, extra: _fontExtra };
}
...
export async function GET() {
  ...
  const { semi: fontSemi, extra: fontExtra } = await getFonts();
  ...
  if (N === 0) {
    ...
    return new ImageResponse(
      (
        <div style={{ width: W, height: H, background: "#000", display: "flex", fontFamily: "Bricolage Grotesque", position: "relative" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", paddingTop: qTop,
            ...
            {/* Quote text */}
            <span style={{
              fontSize: qFont, color: WHITE, fontWeight: 800,
            ...
            <span style={{
              fontSize: 34, color: ORANGE, fontWeight: 600,
            ...
            <span style={{
              fontSize: 24, color: "#2a2a2a", fontWeight: 600, letterSpacing: "0.18em",
            }}>
              {s.toUpperCase()}
            </span>
          </div>
          ...
          <div style={{ position: "absolute", bottom: H - SAFE_BOTTOM + 10,
            ...
            <span style={{ fontSize: 17, color: "#1c1c1c", letterSpacing: "0.08em" }}>{timeStr}</span>
            <SignatureEl />
          </div>
        </div>
      ),
      {
        width: W, height: H,
        fonts: [
          { name: "Bricolage Grotesque", data: fontSemi,  style: "normal", weight: 600 },
          { name: "Bricolage Grotesque", data: fontExtra, style: "normal", weight: 800 },
        ],
        ...
        <div style={{ display: "flex", flexDirection: "column", paddingTop: topPad,
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", height: 50, marginBottom: 12 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
              ...
              <span style={{ fontSize: 26, color: ORANGE, letterSpacing: "0.40em", fontWeight: 600 }}>REMINDERS</span>
            </div>
            <span style={{ fontSize: 26, color: pending.length > 0 ? ORANGE : "#2e2e2e", letterSpacing: "0.08em", fontWeight: 600 }}>
          ...
          {sorted.map((r, i) => (
            <div key={r.id} style={{ display: "flex", alignItems: "center", height: itemHeights[i],
              ...
              <span style={{ fontSize: NUM_PX, color: ORANGE, fontWeight: 600, letterSpacing: "0.06em",
                width: NUM_W, textAlign: "right", flexShrink: 0, opacity: r.done ? 0.40 : 1, lineHeight: 1 }}>
                {String(i + 1).padStart(2, "0")}
              </span>
              <div style={{ width: COL_GAP, flexShrink: 0 }} />
              <span style={{ fontSize, color: r.done ? DONE_C : WHITE, fontWeight: 800, letterSpacing: textLS,
